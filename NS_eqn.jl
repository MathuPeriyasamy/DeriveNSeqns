function NS_eqn(  x₁, x₂, x₃, h₁, h₂, h₃, vars )
...
Πx₁x₁ = simplify( Symbolics.diff2term(expand_derivatives( -P + 2//3*μ/Re*( 2*ex₁x₁ -ex₂x₂ -ex₃x₃ ) )));
Πx₂x₂ = simplify( Symbolics.diff2term(expand_derivatives( -P + 2//3*μ/Re*( 2*ex₂x₂ -ex₁x₁ -ex₃x₃ ) )));
Πx₃x₃ = simplify( Symbolics.diff2term(expand_derivatives( -P + 2//3*μ/Re*( 2*ex₃x₃ -ex₁x₁ -ex₂x₂ ) )));
...
T2 = [  Symbolics.diff2term(expand_derivatives( ( 1/(prod(h)).*( Dx₁(h₂*h₃*Πx₁x₁) + Dx₂(h₁*h₃*Πx₁x₂) + Dx₃(h₁*h₂*Πx₁x₃) ) ) )) + Symbolics.diff2term(expand_derivatives( ( Πx₁x₂./(h₁*h₂)*Dx₂(h₁) + Πx₁x₃./(h₁*h₃)*Dx₃(h₁) - Πx₂x₂./(h₁*h₂)*Dx₁(h₂) - Πx₃x₃./(h₁*h₃)*Dx₁(h₃) ) ));
        Symbolics.diff2term(expand_derivatives( ( 1/(prod(h)).*( Dx₁(h₂*h₃*Πx₁x₂) + Dx₂(h₁*h₃*Πx₂x₂) + Dx₃(h₁*h₂*Πx₂x₃) ) ) )) + Symbolics.diff2term(expand_derivatives( ( Πx₁x₂./(h₁*h₂)*Dx₁(h₂) + Πx₂x₃./(h₂*h₃)*Dx₃(h₂) - Πx₁x₁./(h₁*h₂)*Dx₂(h₁) - Πx₃x₃./(h₂*h₃)*Dx₂(h₃) ) ));
        Symbolics.diff2term(expand_derivatives( ( 1/(prod(h)).*( Dx₁(h₂*h₃*Πx₁x₃) + Dx₂(h₁*h₃*Πx₂x₃) + Dx₃(h₁*h₂*Πx₃x₃) ) ) )) + Symbolics.diff2term(expand_derivatives( ( Πx₁x₃./(h₁*h₃)*Dx₁(h₃) + Πx₂x₃./(h₂*h₃)*Dx₂(h₃) - Πx₁x₁./(h₁*h₃)*Dx₃(h₁) - Πx₂x₂./(h₂*h₃)*Dx₃(h₂) ) ))];
...
ET1 = simplify(expand_derivatives( ρ*Cₚ*( Dt(T) + U[1]/h[1]*Dx₁(T) + U[2]/h[2]*Dx₂(T) + U[3]/h[3]*Dx₃(T)  )  ));

# ET2 = ( Dt(P) + U[1]/h[1]*Dx₁(P) + U[2]/h[2]*Dx₂(P) + U[3]/h[3]*Dx₃(P)  );
ET2 = simplify(expand_derivatives( (γ-1)*Mach^2*( Dt(P) + U[1]/h[1]*Dx₁(P) + U[2]/h[2]*Dx₂(P) + U[3]/h[3]*Dx₃(P)  )  )); # non dimensional

# ET3 = 1/(prod(h))*( Dx₁(κ*h[2]*h[3]/h[1]*Dx₁(T)) + Dx₂(κ*h[1]*h[3]/h[2]*Dx₂(T)) + Dx₃(κ*h[1]*h[2]/h[3]*Dx₃(T)) );
ET3 =  simplify(expand_derivatives( 1/(Pᵣ*Re)*( 1/(prod(h))*( Dx₁(κ*h[2]*h[3]/h[1]*Dx₁(T)) + Dx₂(κ*h[1]*h[3]/h[2]*Dx₂(T)) + Dx₃(κ*h[1]*h[2]/h[3]*Dx₃(T)) ) )  )); # non dimensional

# ET4 = μ*( 2*( ex₁x₁.^2 + ex₂x₂.^2 + ex₃x₃.^2 ) + ex₁x₂.^2 + ex₂x₃.^2 + ex₁x₃.^2 -2/3*( ex₁x₁ + ex₂x₂ + ex₃x₃ ).^2 );

ET4 = simplify(expand_derivatives( (γ-1)*Mach^2/Re*μ*( 2*( ex₁x₁.^2 + ex₂x₂.^2 + ex₃x₃.^2 ) + ex₁x₂.^2 + ex₂x₃.^2 + ex₁x₃.^2 -2//3*( ex₁x₁ + ex₂x₂ + ex₃x₃ ).^2 ) )); # non dimensional
...
# Ceqn = Symbolics.substitute((Symbolics.value(expand_derivatives(Ceqn))),Dict([x₁=>x₁ᵢ,x₂=>x₂ᵢ,x₃=>x₃ᵢ,h₁=>h₁ᵢ,h₂=>h₂ᵢ,h₃=>h₃ᵢ,U₁=>U₁ᵢ,U₂=>U₂ᵢ,U₃=>U₃ᵢ,T=>Tᵢ,P=>Pᵢ,κ=>κᵢ,μ=>μᵢ]));
# Meqn = Symbolics.substitute((Symbolics.value(expand_derivatives(Meqn))),Dict([x₁=>x₁ᵢ,x₂=>x₂ᵢ,x₃=>x₃ᵢ,h₁=>h₁ᵢ,h₂=>h₂ᵢ,h₃=>h₃ᵢ,U₁=>U₁ᵢ,U₂=>U₂ᵢ,U₃=>U₃ᵢ,T=>Tᵢ,P=>Pᵢ,κ=>κᵢ,μ=>μᵢ]));
# Eneqn = Symbolics.substitute((Symbolics.value(expand_derivatives(Eneqn))),Dict([x₁=>x₁ᵢ,x₂=>x₂ᵢ,x₃=>x₃ᵢ,h₁=>h₁ᵢ,h₂=>h₂ᵢ,h₃=>h₃ᵢ,U₁=>U₁ᵢ,U₂=>U₂ᵢ,U₃=>U₃ᵢ,T=>Tᵢ,P=>Pᵢ,κ=>κᵢ,μ=>μᵢ]));
