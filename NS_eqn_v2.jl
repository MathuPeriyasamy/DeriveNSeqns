...
ex1x1 = ( expand_derivatives(( 1/h1*Dx1(U[1]) + U[2]/(h1*h2)*Dx2(h1) + U[3]/(h1*h3)*Dx3(h1) )));
ex2x2 = ( expand_derivatives(( 1/h2*Dx2(U[2]) + U[1]/(h1*h2)*Dx1(h2) + U[3]/(h2*h3)*Dx3(h2) )));
ex3x3 = ( expand_derivatives(( 1/h3*Dx3(U[3]) + U[1]/(h1*h3)*Dx1(h3) + U[2]/(h2*h3)*Dx2(h3) )));

ex1x2 = ( expand_derivatives(( h[2]/h[1]*Dx1(U[2]/h[2]) + h[1]/h[2]*Dx2(U[1]/h[1]) )));
ex2x3 = ( expand_derivatives(( h[3]/h[2]*Dx2(U[3]/h[3]) + h[2]/h[3]*Dx3(U[2]/h[2]) )));
ex1x3 = ( expand_derivatives(( h[3]/h[1]*Dx1(U[3]/h[3]) + h[1]/h[3]*Dx3(U[1]/h[1]) )));

Πx1x1 = ( expand_derivatives(( 2//3*μ/Re*( 2*ex1x1 -ex2x2 -ex3x3 ) )));
Πx2x2 = ( expand_derivatives(( 2//3*μ/Re*( 2*ex2x2 -ex1x1 -ex3x3 ) )));
Πx3x3 = ( expand_derivatives(( 2//3*μ/Re*( 2*ex3x3 -ex1x1 -ex2x2 ) )));

Πx1x2 = ( expand_derivatives(( μ/Re*ex1x2 )));
Πx2x3 = ( expand_derivatives(( μ/Re*ex2x3 )));
Πx1x3 = ( expand_derivatives(( μ/Re*ex1x3 )));
...
ET2 = (( Ec*( Dt(P) + U[1]/h[1]*Dx1(P) + U[2]/h[2]*Dx2(P) + U[3]/h[3]*Dx3(P)  )  )); # non dimensional

ET3 =  (( 1/(Pr*Re)*( 1/(prod(h))*( Dx1(κ*h[2]*h[3]/h[1]*Dx1(T)) + Dx2(κ*h[1]*h[3]/h[2]*Dx2(T)) + Dx3(κ*h[1]*h[2]/h[3]*Dx3(T)) ) )  )); # non dimensional

ET4 = (( Ec/Re*μ*( 2*( ex1x1.^2 + ex2x2.^2 + ex3x3.^2 ) + ex1x2.^2 + ex2x3.^2 + ex1x3.^2 -2//3*( ex1x1 + ex2x2 + ex3x3 ).^2 ) )); # non dimensional
...
Eneqn = expand_derivatives((ET1)) - ((ET2)) - ((ET3)) - ((ET4));

# open("Test2.txt","w") do file 
#     reterm =   string((x1, x2, x3));
#     println(file,"Meqn[1] = ",replace(string(Meqn[1]),string((x1, x2, x3)) =>"") );    
# end         
